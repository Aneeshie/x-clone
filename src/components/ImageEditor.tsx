...
const ImageEditor = ({
  ...
  setModifications: React.Dispatch<React.SetStateAction<Settings>>;
}) => {
  const handleChangeSensitive = (sensitive: boolean) => {
    setModifications((prev) => ({ ...prev, sensitive }));
  };
  const handleChangeType = (type: "original" | "wide" | "square") => {
    setModifications((prev) => ({ ...prev, type }));
  };
  return (
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <div
            className={`cursor-pointer py-1 px-4 rounded-full text-black ${
              modifications.sensitive ? "bg-red-500" : "bg-white"
            }`}
            onClick={() => handleChangeSensitive(!modifications.sensitive)}
          >
            Sensitive
          </div>
        </div>
